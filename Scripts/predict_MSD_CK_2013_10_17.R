...
fileName <- "All Data_MSD_template_final_CK_2013_10_14.txt"
...
boxplot(humSer$value ~ humSer$target, xlab="", ylab = "Signal", main = "CK24: Human serum")
title(xlab = "proteins", mgp=c(7.5, 1, 0))
  # GAPDH profiles
gapdh <- rawData[grep("^GAPDH", rawData$target),]
boxplot(gapdh$value ~ gapdh$sample, xlab = "", main="CK24: GAPDH")
...
CKratios <- lapply(c("CK24_R1", "CK24_R3", "CK24_R6", "CK24_R8"), function(samp){
  cat(samp, '\n')
  ref <- "CK24_N"
